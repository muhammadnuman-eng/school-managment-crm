...
import { Bus, MapPin, Plus, Clock, Users, Edit, Trash2, Eye, Loader2 } from 'lucide-react';
...
export function Transport() {
  ...
  const handleAddRoute = async () => {
    try {
      ...
      if (!formData.routeName || !formData.busRegistrationNumber || !formData.driverName) {
  ...
  const handleAddBus = async () => {
    try {
      ...
      // Refresh buses list
      const busesResponse = await adminService.getBuses();
      setBuses(busesResponse.buses || []);
    } catch (error: any) {
          ...
          <div className="grid grid-cols-2 gap-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input 
                ...
                onChange={(e) => setFormData({ ...formData, busRegistrationNumber: e.target.value })}
              />
              {buses.length > 0 && (
                <p className="text-xs text-gray-500 mt-1">
                  Available buses: {buses.map(b => b.registrationNumber).join(', ')}
                </p>
              )}
            </div>
            <div className="space-y-2">
              ...
              <Input 
                id="driverName" 
                placeholder="e.g., John Smith, Michael Johnson" 
